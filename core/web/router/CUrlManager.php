...
class CUrlManager {
    ...
    public function init() {
        ...
        $this->setRequest(new CHttpRequest);
    }
    ...
    protected function createUrlRule($route, $pattern) {
        if (is_array($route) && isset($route['class']))
            ...
            require_once dirname(__FILE__) . '/' . $this->urlRuleClass . '.php';
            $urlRuleClass = $this->urlRuleClass;//Yii::import($this->urlRuleClass, true);
    ...
    public function parseUrl() {
        if ($this->getUrlFormat() === self::PATH_FORMAT) {
            $rawPathInfo = $this->request->getPathInfo();
            $pathInfo = $this->removeUrlSuffix($rawPathInfo, $this->urlSuffix);
            foreach ($this->_rules as $i => $rule) {
                //if (is_array($rule))
                //    $this->_rules[$i] = $rule = Yii::createComponent($rule);
                if (($r = $rule->parseUrl($this, $this->request, $pathInfo, $rawPathInfo)) !== false)
                    return isset($_GET[$this->routeVar]) ? $_GET[$this->routeVar] : $r;
            }
            if ($this->useStrictParsing)
                throw new CHttpException(404, 'Unable to resolve the request "{route}".', array('{route}' => $pathInfo));
            else
                return $pathInfo;
        }
    ...
    public function getBaseUrl() {
        if ($this->_baseUrl !== null)
            ...
            if ($this->showScriptName)
                $this->_baseUrl = $this->request->getScriptUrl();
            else
                $this->_baseUrl = $this->request->getBaseUrl();
            return $this->_baseUrl;
        }
    }
